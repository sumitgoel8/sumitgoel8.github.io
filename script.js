...
const workingPapers = [
    ...
    },
    {
        ...
        abstract: `For the object reallocation problem, we study whether characterizations of Top Trading Cycles (TTC) based on individual rationality, efficiency, and strategyproofness extend to restricted preference domains. We introduce the top-two condition for domains as a useful criterion for answering this question. The condition requires that, within every subset of objects, any two objects that can each be ranked first can also be ranked as the top two. Our results provide a unifying perspective on existing findings for specific domains, such as the single-peaked and single-dipped domains, while also addressing several previously unexplored domains.`,
...
const publishedPapers = [
    ...
    },
    {
		...
		abstract: `We consider two-player contests with the possibility of ties and study the effect of tie-breaking rules on effort. For a broad family of ratio-form and difference-form contests, with symmetric agents, the equilibrium is symmetric and independent of the tie-breaking rule. With asymmetric agents, the effort of both players is monotone decreasing in the probability that ties are broken in favor of the stronger player — so the effort-maximizing rule commits to breaking ties in favor of the weaker agent. We also study random tie-breaking rules that are unbiased ex-ante and identify conditions under which determining tie-break decisions before the contest leads to greater expected effort than the more common practice of determining this after a tie has occurred.`,
